--Reconexiones a nivel de RGU
CREATE PROCEDURE "T1"."TVI_SP_RGU_RX"
...
AND ORIG_ID = 4 AND "NO_RECONEXIONES" IS NOT NULL;
--**********************************
...
SELECT 
       ...
       P.ID AS ID_PRODUCTOS, CATEGORIA, P.PRODUCTO,
    ...
    AND A.FECHA_CIERRE >= TO_DATE ('16/04/2015', 'DD/MM/YYYY')
    AND A.FECHA_CIERRE <= TO_DATE ('30/04/2015', 'DD/MM/YYYY')
    AND A.TIPO_ACTIVIDAD IN ('Habilitación Manual','Habilitación Red',
    'Orden de Reconexión Manual','Habilitación por Cobranza',
    'Orden de Reconexión')
    ...
    GROUP BY C.CIUDAD_ID, C.PLAY, C.CLIENTE, R.ID, P.CATEGORIA, 
    P.ID, P.PRODUCTO, A.FECHA_CIERRE;
END;
