...
--    fecha de carga anterior                    
      ...
      select 
          '20150802'    as fecha_carga,
           '20150801'          as fecha_cal,
          orig_id
      from ( select top 1 id as orig_id from "T1"."HANA_STG1_CAT_ORG_ID" where id = 3 )
      union
      select
          '20150801'      as fecha_carga,
            '20150731'             as fecha_cal,
          orig_id          
        from ( select top 1 id as orig_id from "T1"."HANA_STG1_CAT_ORG_ID" where id = 3)
        ;
...
--------------------------------------------------------------
   create local temporary column table #t_regio as (   
        select f.hub, r.id as region_id, r.region,e.id
        from (
          select distinct hub   
          from "T1"."F_RGUS_V"
          where fecha_carga = '20150802'
            or  fecha_carga = '20150801'     
        ) f
 ...
 -- truncate table #t_data;    
      insert into #t_data 
	   ...
	   --    trim(upper(f.company)) = upper(emp.empresa)
        where f.fecha_carga = '20150802'
          or  f.fecha_carga = '20150801'
         --
      --    or  f.fecha_carga > '20150801'       
 ;
 
 SELECT DISTINCT  trim(upper( CASE WHEN upper(f.company) = 'TVI MONTERREY' THEN 'TVI' ELSE f.company END ))
 from  "T1"."F_RGUS_V" f
 ;
 
 SELECT * FROM #t_data
 WHERE CUENTA IN (
 	SELECT CLIENTE_ID AS CUENTA 
 	FROM T1.HANA_STG1_CAT_CLIENTE_MIGRADO
 	WHERE REGION = 'NORESTE'
 	)
 ;
      
      
  SELECT * FROM "T1"."F_RGUS_V"
 WHERE CUENTA IN (
 	SELECT CLIENTE_ID AS CUENTA 
 	FROM T1.HANA_STG1_CAT_CLIENTE_MIGRADO
 	WHERE REGION = 'NORESTE'
 ...
 ;         
        ...
        join :#t_regio emp on
          f.hub = emp.hub
        where f.fecha_carga = '20150808'
          or  f.fecha_carga = '20150807';
        */  
      ...
      **************************************************************************************/      
      insert into #t_edo
            ...
            where edo_producto = 1
              ...
              and fuente = 'RGUS'
              and fecha_carga between '20150801' and '20150802'
            group by 
             a.fecha_carga,
             a.cuenta ,
             A.EDO_CUENTA,
