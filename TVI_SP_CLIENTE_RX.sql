--Reconexiones a nivel de cliente
CREATE PROCEDURE "T1"."TVI_SP_CLIENTE_RX"
...
AND ORIG_ID = 4 AND "NO_RECONEXIONES_CLIENTE" IS NOT NULL;
--**********************************
...
SELECT 4 AS ORIG_ID,
         ...
         4 AS ID_EMPRESA,
         MIN (O.NO_ORDEN) 
         --KEEP (DENSE_RANK FIRST ORDER BY O.FECHA_CREACION ASC) 
		 AS NO_ORDEN,
         C.CLIENTE AS CLIENTE,
         C.PLAY AS PLAY_CLI,
         MAP (LENGTH (C.PLAY),  1, '1 PLAY',  2, '2 PLAY',  3, '3 PLAY') 
         AS PLAY_ORD,
